                        ...
                        <tbody>
                            @foreach ($occupancy as $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="#" data-toggle="modal" data-target="#approve{{$item->tenant->badge}}"><span class="badge badge-success">Request Checkout</span></a>
                                    </td>
                            </tr>
                            @endforeach
                        </tbody>
...
<div class="modal fade" id="approve{{ $item->tenant->badge }}" tabindex="-1" role="dialog" aria-labelledby="exampleStandardModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h3 class="modal-title" id="exampleStandardModalLabel"><i class="align-middle mr-2 far fa-fw fa-smile" style="color:blue"></i> Request Checkout for {{ $item->tenant->name }} ?</h3>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body m-3">
            <form class="form-horizontal form-disabled-button" method="POST" action="{{ route('appCheckout.submit', $item->tenant->badge) }}">
                @csrf
                @method('PUT')
                <input type="hidden" name="user_id" value="{{ auth()->id() }}">
                <input type="hidden" name="tenant_id" value="{{ $item->tenant->badge }}">
